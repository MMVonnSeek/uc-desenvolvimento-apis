...
app = FastAPI(title='API de Tarefas com Auth — Aula 6',
              description="""
              ## API de gerenciamento de tarefas Rotas de criação, edição e exclusão exigem autenticação JWT.
              """,
              version='1.0.0',
...
@app.get('/tarefas',
         ...
         description='Retorna a lista completa de tarefas (acesso público).',
         responses={
...
@app.post('/tarefas',
          status_code=201,
          tags=['Tarefas'],
          summary='Criar nova tarefa (requer login)',
          description='Cria uma nova tarefa. Qualquer usuário autenticado pode criar.',
          responses={
              201: {'description': 'Tarefa criada com sucesso'},
              401: {'description': 'Não autenticado'},
          })
def criar(dados: TarefaCreate, email: str = Depends(usuario_logado)):
...
@app.patch('/tarefas/{tarefa_id}',
           tags=['Tarefas'],
           summary='Atualizar tarefa (requer login)',
...
def atualizar(tarefa_id: int, dados: TarefaPatch, email: str = Depends(usuario_logado)):
...
@app.delete('/tarefas/{tarefa_id}',
            tags=['Tarefas'],
            summary='Remover tarefa (requer login)',
...
def remover(tarefa_id: int, email: str = Depends(usuario_logado)):
    global tarefas
